...
export class ChatHistoryService {
  ...
  private static transformApiMessages(apiMessages: ApiMessage[]): Message[] {
    return apiMessages.map((apiMsg: ApiMessage) => {
      ...
      let data: MessageData | undefined = undefined;

      // Parse JSON content if it's from the model
      let done = false;
      if (apiMsg.sender === "model") {
        // Try to parse if it looks like JSON
        if (apiMsg.text.trim().startsWith("{")) {
          try {
            ...
            if (parsedContent.data) {
              data = parsedContent.data as MessageData;
            }
        ...
        // Also check attachment field for data
        if (apiMsg.attachment && apiMsg.attachment.trim() !== "") {
          try {
            const attachmentData = JSON.parse(apiMsg.attachment);
            // If attachment contains data structure, use it
            if (attachmentData.hotels || attachmentData.flights) {
              data = attachmentData as MessageData;
            } else if (attachmentData.data) {
              data = attachmentData.data as MessageData;
            }
          } catch (e) {
    ...
    try {
      ...
      const sessions: ChatSession[] = data.data.chats.map(
        (chat: ChatApiResponse) => {
          const parsedTitle =
            this.parseLastMessageForTitle(chat.last_message) || "";
